# DO SPORTS 실시간 시간표 v10

Netlify + GitHub로 배포하는 DO SPORTS 실시간 시간표입니다.

## v10 변경점

- 학부모 화면을 예시 이미지처럼 어두운 밝은 보드형 시간표 디자인으로 변경
...
`academy-live-schedule-v10` 폴더 자체를 올리지 말고, 그 안의 파일과 폴더를 올리세요.
...
- 전체 학부모 화면을 밝은 테마로 변경했습니다.
- 시간표 왼쪽 칸은 `15:00대`처럼 시간대 기준으로 표시합니다.
- 수업 카드 안에 `15:20~16:10`처럼 실제 시작/종료 시간을 정확히 표시합니다.
- 정각에 끝나지 않는 수업도 한눈에 구분할 수 있도록 카드 상단에 시간 배지를 추가했습니다.
